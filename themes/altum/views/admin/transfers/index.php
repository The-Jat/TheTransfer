...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0 text-truncate"><i class="fas fa-fw fa-xs fa-paper-plane text-primary-900 mr-2"></i> <?= l('admin_transfers.header') ?></h1>
        ...
        <div>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/transfers?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/transfers?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
...
<form id="table" action="<?= SITE_URL . 'admin/transfers/bulk' ?>" method="post" role="form">
        ...
        <table class="table table-custom">
            ...
            <?php foreach($data->transfers as $row): ?>
                <tr>
                    ...
                    <td class="text-nowrap">
                        <span title="<?= $row->name ?>"><?= string_truncate($row->name, 32) ?></span> <a href="<?= url('transfer-redirect/' . $row->transfer_id) ?>" rel="noreferrer" target="_blank"><i class="fas fa-fw fa-xs fa-external-link-alt text-muted ml-1"></i></a>
                    </td>

                    <td class="text-nowrap">
                        <a href="<?= url('admin/files?transfer_id=' . $row->transfer_id) ?>" class="badge badge-info">
                            <?= nr($row->total_files) ?>
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/transfers/admin_transfer_dropdown_button.php', ['id' => $row->transfer_id, 'resource_name' => $row->name]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>

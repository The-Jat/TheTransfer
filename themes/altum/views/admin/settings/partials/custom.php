...
<div>
    ...
    <div class="form-group">
        <label for="head_js_transfers"><i class="fab fa-fw fa-sm fa-js text-muted mr-1"></i> <?= l('admin_settings.custom.head_js_transfers') ?></label>
        <textarea id="head_js_transfers" name="head_js_transfers" class="form-control" data-code-editor data-mode="htmlmixed"><?= settings()->custom->head_js_transfers ?></textarea>
        <small class="form-text text-muted"><?= sprintf(l('global.variables'), '<code>' . implode('</code> , <code>',  ['{{WEBSITE_TITLE}}', '{{USER:NAME}}', '{{USER:EMAIL}}', '{{USER:CONTINENT_NAME}}', '{{USER:COUNTRY_NAME}}', '{{USER:CITY_NAME}}', '{{USER:DEVICE_TYPE}}', '{{USER:OS_NAME}}', '{{USER:BROWSER_NAME}}', '{{USER:BROWSER_LANGUAGE}}', '{{USER:USER_ID}}', '{{USER:PLAN_ID}}']) . '</code>') ?></small>
    </div>

    <div class="form-group">
        <label for="head_css_transfers"><i class="fab fa-fw fa-sm fa-css3 text-muted mr-1"></i> <?= l('admin_settings.custom.head_css_transfers') ?></label>
        <textarea id="head_css_transfers" name="head_css_transfers" class="form-control" data-code-editor data-mode="css"><?= settings()->custom->head_css_transfers ?></textarea>
    </div>

    <div class="form-group">
        <label for="body_content_transfers"><i class="fab fa-fw fa-sm fa-html5 text-muted mr-1"></i> <?= l('admin_settings.custom.body_content_transfers') ?></label>
        <textarea id="body_content_transfers" name="body_content_transfers" class="form-control" data-code-editor data-mode="htmlmixed"><?= settings()->custom->body_content_transfers ?></textarea>
        <small class="form-text text-muted"><?= l('admin_settings.custom.body_content_help') ?></small>
    </div>
</div>

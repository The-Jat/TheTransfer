    ...
    <div class="card-body">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex align-items-center">
                <h3 class="h5 text-truncate m-0"><?= isset($_GET['continent_code']) ? sprintf(l('transfer_statistics.country_from_continent_code'), get_continent_from_continent_code($data->continent_code)) : l('global.countries') ?></h3>

                <div class="ml-2">
                    <span data-toggle="tooltip" title="<?= l('transfer_statistics.country_help') ?>">
            ...
            <div class="d-flex align-items-center col-auto p-0">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right d-print-none">
                        <a href="<?= url('transfer-statistics/' . $data->transfer->transfer_id . '?' . \Altum\Router::$original_request_query . '&export=csv') ?>" target="_blank" class="dropdown-item">
                            <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                        </a>
                        <a href="<?= url('transfer-statistics/' . $data->transfer->transfer_id . '?' . \Altum\Router::$original_request_query . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
                    ...
                    <div class="d-flex justify-content-between mb-1">
                        <div class="text-truncate">
                            ...
                            <?php if($row->country_code): ?>
                                <a href="<?= url('transfer-statistics/' . $data->transfer->transfer_id . '?type=city_name&country_code=' . $row->country_code . '&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>" title="<?= $row->country_code ?>" class=""><?= get_country_from_country_code($row->country_code) ?></a>
                            <?php else: ?>

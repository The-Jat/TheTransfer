...
class Uploads {
    ...
    public static function delete_uploaded_file_and_potential_residue($already_existing_file_name, $uploads_file_key, $upload_id = null) {
        if(!empty($already_existing_file_name)) {
            /* Offload deleting */
            if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                $s3 = new \Aws\S3\S3Client(get_aws_s3_config());
                ...
                if($upload_id) {
                    try {
                        ...
                        $command = $s3->getCommand('AbortMultipartUpload', [
                            'Bucket' => settings()->offload->storage_name,
                            'Key' => UPLOADS_URL_PATH . Uploads::get_path($uploads_file_key) . $already_existing_file_name,
                            'UploadId' => $upload_id,
                        ]);

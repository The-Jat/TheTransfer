...
class ApiDomains extends Controller {
    ...
    private function post() {
        ...
        if(in_array($_POST['host'], settings()->transfers->blacklisted_domains)) {
            $this->response_error(l('transfer.error_message.blacklisted_domain'), 401);
        }

        if(!empty($_POST['custom_index_url']) && in_array(get_domain_from_url($_POST['custom_index_url']), settings()->transfers->blacklisted_domains)) {
            $this->response_error(l('transfer.error_message.blacklisted_domain'), 401);
        }

        if(!empty($_POST['custom_not_found_url']) && in_array(get_domain_from_url($_POST['custom_not_found_url']), settings()->transfers->blacklisted_domains)) {
            $this->response_error(l('transfer.error_message.blacklisted_domain'), 401);
        }

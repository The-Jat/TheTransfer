...
class TransferDownloads extends Controller {
    ...
    public function index() {
        ...
        switch($type) {
            case 'overview':
                ...
                $pageviews_result = database()->query("
                    SELECT
                        COUNT(`download_id`) AS `pageviews`,
            ...
            case 'entries':
                ...
                $filters->set_default_order_by('download_id', $this->user->preferences->default_order_type ?? settings()->main->default_order_type);
                $filters->set_default_results_per_page($this->user->preferences->default_results_per_page ?? settings()->main->default_results_per_page);

                /* Prepare the paginator */
                $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `downloads` WHERE `transfer_id` = {$transfer->transfer_id} AND (`datetime` BETWEEN '{$datetime['query_start_date']}' AND '{$datetime['query_end_date']}') {$filters->get_sql_where()}")->fetch_object()->total ?? 0;
                $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('transfer-downloads/' . $transfer->transfer_id . '?type=' . $type . '&start_date=' . $datetime['start_date'] . '&end_date=' . $datetime['end_date'] . $filters->get_get() . '&page=%d')));
        ...
        $view = new \Altum\View('transfer-downloads/index', (array) $this);
    ...
    public function reset() {
        ...
        $transfer_id = (int) query_clean($_POST['transfer_id']);
        $datetime = \Altum\Date::get_start_end_dates_new($_POST['start_date'], $_POST['end_date']);

        /* Make sure the transfer id is created by the logged in user */
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            database()->query("DELETE FROM `downloads` WHERE `transfer_id` = {$transfer->transfer_id} AND (`datetime` BETWEEN '{$datetime['query_start_date']}' AND '{$datetime['query_end_date']}')");
        ...
        redirect('transfer-downloads/' . $transfer->link_id);

...
class TransferStatistics extends Controller {
    ...
    public function index() {
        ...
        switch($type) {
            ...
            case 'entries':
                ...
                $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `statistics` WHERE `transfer_id` = {$transfer->transfer_id} AND (`datetime` BETWEEN '{$datetime['query_start_date']}' AND '{$datetime['query_end_date']}') {$filters->get_sql_where()}")->fetch_object()->total ?? 0;
                $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('transfer-statistics/' . $transfer->transfer_id . '?type=' . $type . '&start_date=' . $datetime['start_date'] . '&end_date=' . $datetime['end_date'] . $filters->get_get() . '&page=%d')));
        ...
        $data['type'] = $type;
        $view = new \Altum\View('transfer-statistics/statistics_' . $type, (array) $this);
        ...
        $view = new \Altum\View('transfer-statistics/index', (array) $this);
    ...
    public function reset() {
        ...
        if(\Altum\Teams::is_delegated() && !\Altum\Teams::has_access('delete.transfers')) {
            ...
            redirect('transfer-statistics/' . $transfer->transfer_id);
        }
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            database()->query("DELETE FROM `statistics` WHERE `transfer_id` = {$transfer->transfer_id} AND (`datetime` BETWEEN '{$datetime['query_start_date']}' AND '{$datetime['query_end_date']}')");
        ...
        redirect('transfer-statistics/' . $transfer->link_id);

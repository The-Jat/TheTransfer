...
class AdminStatistics extends Controller {
    ...
    protected function transfers() {
        ...
        while($row = $result->fetch_object()) {
            ...
            $transfers_chart[$row->formatted_date] = [
                'transfers' => $row->total
            ];

            $total['transfers'] += $row->total;
        }
    ...
    protected function downloads() {
        ...
        while($row = $result->fetch_object()) {
            ...
            $downloads_chart[$row->formatted_date] = [
                'downloads' => $row->total
            ];

            $total['downloads'] += $row->total;
        }

        $downloads_chart = get_chart_data($downloads_chart);

        return [
            'total' => $total,
            'downloads_chart' => $downloads_chart,
        ];
